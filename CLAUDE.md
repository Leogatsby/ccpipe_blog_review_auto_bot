# 청청배관 블로그협찬 초안생성봇

## 프로젝트 개요
"청청배관"이라는 하수구 및 인테리어 산업 서비스를 직접 운영 중이며, 해당 블로그 협찬단이 리뷰 글을 편하게 작성할 수 있도록 맞춤형 초안을 자동 생성하고 완성된 글을 Notion 데이터베이스에 업로드하는 도구입니다.
※ 청청배관의 업체명 및 CTA 플레이스 링크 등 기본 정보는 `.claude/commonData/기본나의업체정보/업체정보.md`에 저장되어 있습니다.

## 에이전트 워크플로우

| 순서 | 에이전트 | 역할 | 데이터파일 생성 흐름 |
|------|----------|------|-------------|
| 00 | SystemSetter | 노션 DB(키워드산업, 지역키워드)를 CSV로 갱신 | `.claude/commonData/00.초기노션DB시스템세팅/` (산업키워드.csv, 지역키워드.csv 갱신) |
| 01 | Researcher | 네이버 API로 키워드 적절성 검증 | 네이버 API 검증 → `에이전트프로세스보고/01.Researcher/` |
| 02 | Writer | 검증된 키워드 기반 블로그 초안 작성 | 블로그 초안 작성 → `에이전트프로세스보고/02.Writer/` |
| 03 | Reviewer | 초안 검수 및 수정 | 검수/수정 → `에이전트프로세스보고/03.Reviewer/` |
| 04 | Publisher | 노션 DB에 최종본 업로드 | 노션 업로드 → `에이전트프로세스보고/04.Publisher/` |

## 결과물 폴더 구조

폴더명 형식: `날짜_지역구_산업업종_산업업종키워드_협찬`
최종 글 파일명: 블로그 제목 그대로 `.md`로 저장

- 지역구, 산업업종, 산업업종키워드는 `.claude/commonData/00.초기노션DB시스템세팅/지역키워드.csv`, `.claude/commonData/00.초기노션DB시스템세팅/산업키워드.csv`에서 가져온다

```
최종결과물/
└── 2026-05-24_관악구_배관_변기막힘_협찬/
    ├── 관악구 봉천동 변기막힘 업체 강력 추천해요.md    ← 최종 블로그 글
    └── 에이전트프로세스보고/
        ├── 01.Researcher/
        ├── 02.Writer/
        ├── 03.Reviewer/
        └── 04.Publisher/
```

## 설정
- API 인증 정보: `.claude/commonData/API설정.md`
- MCP 서버 설정: `.claude/commonData/settings.json`
- 노션 동기화 데이터: `.claude/commonData/00.초기노션DB시스템세팅/산업키워드.csv`, `.claude/commonData/00.초기노션DB시스템세팅/지역키워드.csv`
- 기본 업체 정보: `.claude/commonData/기본나의업체정보/업체정보.md`
